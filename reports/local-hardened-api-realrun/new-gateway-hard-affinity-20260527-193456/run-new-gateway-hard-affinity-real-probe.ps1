...
try {
  ...
  $responseBody = [ordered]@{ model = $Model; input = "Reply with exactly OK."; stream = $false }
  $headers = @{ "x-codex-turn-state" = $turnState }

  Write-Log "sending initial sticky Responses request"
  $initial = Invoke-JsonHttp -Method POST -Uri "$($ready.baseUrl)/responses" -ApiKey $ready.apiKey -Body $responseBody -ExtraHeaders $headers -TimeoutSeconds 180
  Write-Log "initial sticky Responses status=$($initial.statusCode) timedOut=$($initial.timedOut)"
  ...
  if ($quotaObserved) {
    ...
    $second = Invoke-JsonHttp -Method POST -Uri "$($ready.baseUrl)/responses" -ApiKey $ready.apiKey -Body $responseBody -ExtraHeaders $headers -TimeoutSeconds $HardAffinityClientTimeoutSeconds
